def q1_check(models_dict):
    ...
    if models["logistic regression"] != models_dict["logistic regression"]:
        print(
            "Oops!  In most cases, you will only want to use logistic regression for classification problems."
        )

    if models["linear regression"] != models_dict["linear regression"]:
        print(
            "Oops!  Linear regression should actually only be used in regression cases. Try again."
        )

    if (
        (models["decision trees"] != models_dict["decision trees"])
        or (models["random forest"] != models_dict["random forest"])
        or (models["adaptive boosting"] != models_dict["adaptive boosting"])
    ):
        print(
...
def q6_check(metrics):
    ...
    if (
        ...
        or (metrics["recall"] != metrics["recall"])
        or (metrics["accuracy"] != metrics["accuracy"])
        or (metrics["area_under_curve"] != metrics["area_under_curve"])
    ):
        print(
